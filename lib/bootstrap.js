module.exports = function bootstrap(options) {
  ...
  app.resolveScripts = function (scripts, default_path) {
    ...
    _.each(scripts, function (script) {
      var path = bot.options.dir + default_path + script + '.js';

      if (fs.existsSync(path)) {
        file_paths[script] = path;
      } else {
        // BotDylan folder fallback
        path = __dirname + '/..' + default_path + script + '.js';

        if (fs.existsSync(path)) {
          file_paths[script] = path;
        } else {
          throw new Error("Script '" + script + "' ('" +  path + "') not found!");
        }
      }
    });
